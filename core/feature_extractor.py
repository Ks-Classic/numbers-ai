...
def add_keisen_pattern_features(
    features: Dict[str, float],
    previous_winning: str,
    previous_previous_winning: str,
    target: Target
) -> Dict[str, float]:
    """罫線パターンIDを特徴量として追加する
    
    各桁の前回・前々回の数字の組み合わせをエンコードして特徴量として追加します。
    これにより、罫線パターン固有の予測傾向を学習できるようになります。
    
    Args:
        features: 既存の特徴量辞書
        previous_winning: 前回の当選番号（文字列、例: "149"）
        previous_previous_winning: 前々回の当選番号（文字列、例: "652"）
        target: 対象（'n3' または 'n4'）
    
    Returns:
        罫線パターンID特徴量を追加した特徴量辞書
    ...
    Examples:
        ...
        ...     features, "149", "652", "n3"
        ... )
        >>> features['keisen_pattern_百の位']  # 前々回=6, 前回=1 → 61
        61.0
        >>> features['keisen_pattern_十の位']  # 前々回=5, 前回=4 → 54
        54.0
        >>> features['keisen_pattern_一の位']  # 前々回=2, 前回=9 → 29
        29.0
    """
    # 対象に応じた桁名を定義
    if target == 'n3':
        column_names = ['百の位', '十の位', '一の位']
        expected_length = 3
    else:  # n4
        column_names = ['千の位', '百の位', '十の位', '一の位']
        expected_length = 4
    
    # 先頭0を補完（数値として読み込まれた場合に備えて）
    ...
    for i, column_name in enumerate(column_names):
        ...
        try:
            ...
            # エラー時はデフォルト値（0）を使用
            previous_digit = 0
            previous_previous_digit = 0
        
        # 罫線パターンIDを計算（前々回の数字 × 10 + 前回の数字）
        # 範囲: 0-99（前々回0-9、前回0-9の組み合わせ）
        pattern_id = previous_previous_digit * 10 + previous_digit
        
        # 特徴量名を作成（例: keisen_pattern_百の位）
        feature_name = f'keisen_pattern_{column_name}'
        features[feature_name] = float(pattern_id)
...
def add_weekday_feature(
    ...
    Examples:
        >>> features = {}
        >>> features = add_weekday_feature(features, 2)  # 水曜日
        >>> features['weekday']
        2.0
    """
    if weekday is not None and 0 <= weekday <= 6:
        ...
        # デフォルト値（不明な場合）
        features['weekday'] = 0.0
    
    return features


def calculate_chart_reliability_features(
    grid: List[List[Optional[int]]],
    rows: int,
    cols: int,
    winning: Optional[str]  # 実際の当選番号（学習時のみ使用、推論時はNone）
) -> Dict[str, float]:
    """予測表の信頼性特徴量を計算する（学習時のみ）
    
    当選番号の各数字が予測表上に存在するか、連続して存在するかを判定します。
    これにより、予測表の信頼性を評価し、「この回は買わない」判断が可能になります。
    
    Args:
        grid: 予測表グリッド（1-indexed）
        rows: 行数（1-indexed）
        cols: 列数（1-indexed）
        winning: 実際の当選番号（例: "149"、学習時のみ使用、推論時はNone）
    
    Returns:
        信頼性特徴量の辞書
        - all_winning_digits_exist: すべての数字が存在するか（0 or 1）
        - winning_digits_exist_ratio: 存在する数字の割合（0-1）
        - consecutive_winning_digits_exist: 連続して存在するか（0 or 1）
        - winning_digits_min_distance: 数字間の最小距離
    
    Examples:
        >>> grid = [[None, None, None], [None, 1, 4], [None, 9, None]]
        >>> features = calculate_chart_reliability_features(grid, 2, 3, "149")
        >>> features['all_winning_digits_exist']  # すべて存在する
        1.0
        >>> features['winning_digits_exist_ratio']  # すべて存在する
        1.0
    """
    features = {}
    ...
    if winning:
        # 当選番号の各数字を取得
        winning_str = str(winning).replace('.0', '')
        digits = [int(d) for d in winning_str]
        
        # 各数字が予測表上に存在するか
        digit_exists = []
        digit_positions_list = []
        for digit in digits:
            positions = get_digit_positions(grid, rows, cols, digit)
            exists = len(positions) > 0
            digit_exists.append(exists)
            digit_positions_list.append(positions)
        
        # すべての数字が存在するか
        features['all_winning_digits_exist'] = 1.0 if all(digit_exists) else 0.0
        
        # 存在する数字の割合
        features['winning_digits_exist_ratio'] = sum(digit_exists) / len(digits) if digits else 0.0
        
        # 連続して存在するか（隣接する数字の位置が近い場合、連続していると判定）
        consecutive_pairs = []
        all_min_distances = []
        
        for i in range(len(digit_positions_list) - 1):
            pos1_list = digit_positions_list[i]
            pos2_list = digit_positions_list[i + 1]
            ...
            # 距離が2以下なら連続していると判定（隣接または斜め隣接）
            is_consecutive = min_distance <= 2.0
            consecutive_pairs.append(is_consecutive)
            
            if min_distance != float('inf'):
                all_min_distances.append(min_distance)
        
        # 連続しているペアが存在するか
        features['consecutive_winning_digits_exist'] = 1.0 if any(consecutive_pairs) else 0.0
        
        # 数字間の最小距離
        features['winning_digits_min_distance'] = min(all_min_distances) if all_min_distances else 999.0
    else:
        # 推論時はデフォルト値（計算不要）
        features['all_winning_digits_exist'] = 0.0
        features['winning_digits_exist_ratio'] = 0.0
        features['consecutive_winning_digits_exist'] = 0.0
        features['winning_digits_min_distance'] = 999.0
