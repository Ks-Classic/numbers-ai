...
2. 基準回号を設定（第6758回）
3. 直近100回分のデータを抽出（第6659回〜第6758回）
4. 学習用データセットとして保存（`train_data_100.csv`）

**データ形式**:
- 各回号には「n3_winning」（3桁の当選番号）と「n4_winning」（4桁の当選番号）が記録されている
- 例: 第6758回のn3_winning = "149"、n4_winning = "3782"

**出力**: `data/train_data_100.csv`（100行の学習用データ）
...
**軸数字予測モデルの学習データ**:
```
各回号 × 4パターン × 10数字（0-9） = 4,000サンプル

例: 第6758回、パターンA1、数字5の場合
  入力: 特徴量ベクトル（21次元）
...
**組み合わせ予測モデルの学習データ**:
```
各回号 × 4パターン × サンプリングされた組み合わせ = 数万サンプル

例: 第6758回、パターンA1、組み合わせ"149"の場合
  入力: 特徴量ベクトル（72次元）
     - 組み合わせに関する特徴量、パターンID
  出力: ラベル（0または1）
...
**ポイント**:
- 過去100回分のデータを全て使って、各回号ごとに予測表を生成
...
**重要なポイント**:
- **学習時**: 過去100回分のデータで「予測表の特徴量」と「実際の当選結果」の関係を学習
