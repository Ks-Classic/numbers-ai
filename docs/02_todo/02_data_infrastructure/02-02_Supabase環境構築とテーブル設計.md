# 02-02 Supabase環境構築とテーブル設計

## 概要
Supabaseプロジェクトを作成し、PostgreSQLデータベースのテーブル設計とマイグレーションを実装する。CSVベースのデータ管理からSupabase（PostgreSQL）への移行準備を行う。

## 前提条件

- AI精度改善が完了していること（優先度が高いため）
...
- テーブル設計は既存のCSVデータ構造を考慮する
- インデックスを適切に設定してクエリパフォーマンスを最適化する
