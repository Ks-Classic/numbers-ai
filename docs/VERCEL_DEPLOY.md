...
- ✅ `api/py/requirements.txt` (scikit-learn除外)
- ✅ `api/py/axis.py` (libgomp手動ロード処理あり)
- ✅ `api/py/combination.py` (libgomp手動ロード処理あり)
...
- `data/models/*_lgb.txt`: 学習済みモデル（LightGBMネイティブ形式）
- `data/models/*_keys.json`: モデル特徴量キー

**注意:** `.pkl` ファイルはサイズと互換性の問題から**使用禁止**です。Vercelにはデプロイされません。

### Python関数の制約

Vercelのビルド時間とパッケージサイズ制限を回避するため、以下の制約を設けています：

1.  **Pandas使用禁止**: `api/py` 以下のコードでは `pandas` をインポートしてはいけません。代わりに `csv` 標準モジュールを使用してください。
2.  **Pickle使用禁止**: `pickle` モジュールや `.pkl` ファイルの読み込みは禁止です。
3.  **軽量ライブラリ**: `numpy` と `lightgbm` のみ使用可能です。`scikit-learn` は使用しません。
...
- [Git巨大ファイル対策](./03_fixes/git-large-files-prevention-2025-12-17.md): Gitリポジトリ最適化の記録
