...
import { 
  ...
  type N3KeisenMaster,
  type N4KeisenMaster,
  type PredictedDigits
} from '@/lib/data-loader';
import { loadKeisenMasterByType, type KeisenMasterType } from './keisen-master-loader';
...
export async function generateChart(
  ...
  try {
    ...
    return {
      ...
      tempList, // templist（4桁単位で最小値から順に重複せずに選択した順序）
      centerZeroPlaced
    };
  } catch (error) {
 ...
 * keisen_master.jsonまたはkeisen_master_new.jsonを参照して各桁の予測出目を取得し、結合する。
 * 
 * @param roundNumber 回号
 * @param target 対象（'n3' または 'n4'）
 * @param keisenMasterType 罫線マスターの種類（'current' または 'new'）
 ...
 * @param keisenMasterType 罫線マスターの種類（'current' または 'new'）
 * @returns 予測出目の配列
 * @throws ChartGenerationError データ読み込みエラーまたはデータが見つからない場合
 */
async function getPredictedDigitsFromKeisenMaster(
  target: Target,
  ...
  if (target === 'n3') {
    ...
    if (columnName === '百の位' || columnName === '十の位' || columnName === '一の位') {
      ...
      throw new ChartGenerationError(`N3では${columnName}は使用できません`);
    }
  } else {
 ...
 * @returns メイン行の配列
 */
function buildMainRows(nums: number[]): { mainRows: MainRow[]; tempList: number[] } {
