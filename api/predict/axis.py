...
def handler(event: Dict[str, Any], context: Any = None) -> Dict[str, Any]:
    ...
    """
    print("=" * 60)
    print("[AXIS HANDLER] 関数が呼ばれました")
    print(f"[AXIS HANDLER] event type: {type(event)}")
    print(f"[AXIS HANDLER] event keys: {event.keys() if isinstance(event, dict) else 'N/A'}")
    print(f"[AXIS HANDLER] event: {event}")
