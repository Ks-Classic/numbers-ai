...
for target in ['n3', 'n4']:
    ...
    # 軸数字モデルではrehearsal_distanceが軸数字との距離を表す
    ...
    # 関係性特徴量を抽出
    axis_distances = X_all[:, rehearsal_distance_idx]
    overlap_counts = X_all[:, overlap_count_idx]
    inverse_ratios = X_all[:, inverse_ratio_idx]
    
    # 当選（label=1）と非当選（label=0）に分ける
    # 軸数字が存在する場合のみ（999.0でない場合）
    valid_mask = axis_distances != 999.0
    
    if valid_mask.sum() == 0:
        print(f"警告: {target}のデータで軸数字が存在するサンプルがありません")
        continue
    
    axis_distances_valid = axis_distances[valid_mask]
    overlap_counts_valid = overlap_counts[valid_mask]
    inverse_ratios_valid = inverse_ratios[valid_mask]
    y_valid = y_all[valid_mask]
    
    # 統計量を計算
    # 当選番号に重点を置くため、当選時（won）を先に分析
    won_mask = y_valid == 1
    lost_mask = y_valid == 0
    ...
    stats = {
        ...
        'won_rate': float(won_mask.sum() / len(y_valid)) if len(y_valid) > 0 else 0.0,
        'axis_distance': {
            'all_mean': float(np.mean(axis_distances_valid)),
            'all_std': float(np.std(axis_distances_valid)),
            'all_median': float(np.median(axis_distances_valid)),
            'won_mean': float(np.mean(axis_distances_valid[won_mask])) if won_mask.sum() > 0 else None,
            'won_std': float(np.std(axis_distances_valid[won_mask])) if won_mask.sum() > 0 else None,
            'won_median': float(np.median(axis_distances_valid[won_mask])) if won_mask.sum() > 0 else None,
            'lost_mean': float(np.mean(axis_distances_valid[lost_mask])) if lost_mask.sum() > 0 else None,
            'lost_std': float(np.std(axis_distances_valid[lost_mask])) if lost_mask.sum() > 0 else None,
            'lost_median': float(np.median(axis_distances_valid[lost_mask])) if lost_mask.sum() > 0 else None,
            'difference_mean': float(np.mean(axis_distances_valid[won_mask]) - np.mean(axis_distances_valid[lost_mask])) if won_mask.sum() > 0 and lost_mask.sum() > 0 else None,
        },
        'overlap_count': {
            ...
            'difference_mean': float(np.mean(overlap_counts_valid[won_mask]) - np.mean(overlap_counts_valid[lost_mask])) if won_mask.sum() > 0 and lost_mask.sum() > 0 else None,
        },
        'inverse_ratio': {
            ...
            'difference_mean': float(np.mean(inverse_ratios_valid[won_mask]) - np.mean(inverse_ratios_valid[lost_mask])) if won_mask.sum() > 0 and lost_mask.sum() > 0 else None,
        }
    }
    ...
    print(f"  当選サンプル数: {stats['won_samples']} ({stats['won_rate']*100:.1f}%)")
    print(f"  非当選サンプル数: {stats['lost_samples']}")

# 結果を保存
import json
output_file = REPORT_DIR / 'axis_analysis_stats.json'
