#!/usr/bin/env python3
"""
既存のkeisen_master.jsonの予測出目が6桁以上のパターンを分析
それらが本当に同率なのかを検証する
"""
...
print(f'予測出目が6桁以上のパターン数: {len(long_predictions)}')
print()

# データを読み込んで、これらのパターンが本当に同率なのか検証
...
for i, item in enumerate(sorted(long_predictions, key=lambda x: x['count'], reverse=True)[:10], 1):
    ...
    print(f'  keisen_master.jsonの予測出目 ({len(predictions)}桁): {predictions}')
    ...
    if len(set(prediction_counts)) == 1:
        print(f'  → 予測出目の数字は全て同率（{prediction_counts[0]}回）✅')
    else:
        print(f'  → 予測出目の数字の出現回数: {prediction_counts}')
        print(f'  → 同率ではない ⚠️')
    print()
    print('-' * 60)
    print()
