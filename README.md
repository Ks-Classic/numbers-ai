...
**注意**: 
- FastAPIサーバーは別途デプロイが必要です（Railway、Cloud Run等）。詳細は `VERCEL_DEPLOY.md` を参照してください。
- **CUBE生成機能はVercelでそのまま動作します**（FastAPIサーバー不要）。詳細は `VERCEL_DEPLOY.md` と `VERCEL_CUBE_DEPLOY_CHECKLIST.md` を参照してください。
